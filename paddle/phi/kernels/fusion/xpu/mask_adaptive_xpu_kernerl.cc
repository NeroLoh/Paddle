...
void MaskAdaptiveXPUKernel(const Context& ctx,
  ...
  for (auto batch_idx = 0; batch_idx < batch_size; batch_idx++) {
    ...
    std::cout << "cur_batch_seq_len:" << cur_batch_seq_len << std::endl;
  }
  auto* seq_lod_ptr = ctx.template HostAlloc<int>(SeqLod);
  ...
  auto* pad_seq_len_ptr = ctx.template HostAlloc<int>(PadSeqLen);
  pad_seq_len_ptr[0] = pad_seq_len;
}

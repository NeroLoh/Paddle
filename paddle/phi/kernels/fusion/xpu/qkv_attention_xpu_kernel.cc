...
void QKVAttentionXPUKernelImpl(const Context& ctx,
  ...
  //     qk_max.get_ptr() == nullptr ? nullptr :
  //     qk_max.get_ptr()->data<float>();
  const float* qk_max_data = nullptr;
  const float* qkv_max_data =
      qkv_max.get_ptr() == nullptr ? nullptr : qkv_max.get_ptr()->data<float>();

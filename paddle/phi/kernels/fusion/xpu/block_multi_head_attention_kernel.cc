...
void qkv_split_rope_kernel(const Context& xpu_ctx,
  ...
  float* rotary_pos_emb_buffer =
      RAII_GUARD.alloc_l3_or_gm<float>(rotary_emb.numel() * 2);
  r = baidu::xpu::api::concat<float>(
      xpu_ctx.x_context(),
      {rotary_emb.data<float>(), rotary_emb.data<float>()},
      rotary_pos_emb_buffer,
      {{2, 1, max_seq_len, dim_head / 2}, {2, 1, max_seq_len, dim_head / 2}},
      3);
  PD_CHECK(r == 0, "baidu::xpu::api::concat failed.");
...
void BlockMultiheadAttentionXPUKernel(
    ...
    DenseTensor* cache_k_per_batch_maxs_out,
    DenseTensor* cache_v_per_batch_maxs_out) {
  ...
  int r = xpu::constant<XPUType>(xpu_context,
                                 ...
                                 0);
  PADDLE_ENFORCE_EQ(
      r, 0, phi::errors::InvalidArgument("xpu::constant run failed"));
  ...
  key_cache_out->ShareBufferWith(key_cache);
  value_cache_out->ShareBufferWith(value_cache);
  cache_k_per_batch_maxs_out->ShareBufferWith(cache_k_per_batch_maxs);
  cache_v_per_batch_maxs_out->ShareBufferWith(cache_v_per_batch_maxs);
  ...
  if (max_enc_len_this_time_data > 0) {
    ...
    if (cache_k_quant_scales && dynamic_cachekv_quant) {
      ...
      int ret = xpu::reshape_cached_kv<XPUType, XPUType, int32_t>(
          ...
          reinterpret_cast<XPUType*>(key_cache_out->data<T>()),
      ...
      PADDLE_ENFORCE_EQ(
          ret,
          0,
          phi::errors::InvalidArgument("xpu::reshape_cached_kv run failed"));
      ...
      ret = xpu::copy2d<float>(xpu_context,
                               p_batch_max_ptrs,
                               cache_k_per_batch_maxs_out->data<float>(),
                               bsz,
                               1,
                               MAXPTR_N,
                               1);
      ret = xpu::reshape_cached_kv<XPUType, XPUType, int32_t>(
          ...
          reinterpret_cast<XPUType*>(value_cache_out->data<T>()),
          block_tables.data<int>(),
          lods,
          start_token_ctx_VP,
          ordered_index_ctx_VP,
      ...
      ret = xpu::copy2d<float>(xpu_context,
                               p_batch_max_ptrs,
                               cache_v_per_batch_maxs_out->data<float>(),
                               bsz,
                               1,
                               MAXPTR_N,
                               1);
  ...
  if (max_dec_len_this_time_data > 0) {
    ...
    PADDLE_ENFORCE_EQ(
        ret, 0, phi::errors::InvalidArgument("xpu::constant run failed"));
    ...
    ret = xpu::max<float>(xpu_context,
                          cache_k_per_batch_maxs.data<float>(),
                          p_batch_max_ptrs_fill,
                          cache_k_per_batch_maxs_out->data<float>(),
                          bsz * MAXPTR_N);

    ret = xpu::findmax<float>(xpu_context,
                              cache_k_per_batch_maxs_out->data<float>(),
                              p_cache_k_max_data,
                              bsz * MAXPTR_N);
    ret = xpu::reshape_cached_kv<XPUType, XPUType, int32_t>(
    ...
    PADDLE_ENFORCE_EQ(
        ret,
        0,
        phi::errors::InvalidArgument("xpu::qkv_paged_attention run failed"));
  }
  VLOG(3) << "decoder done";
}
}  // namespace fusion

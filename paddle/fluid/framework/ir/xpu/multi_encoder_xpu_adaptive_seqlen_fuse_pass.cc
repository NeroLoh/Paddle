...
int MultiEncoderXPUAdaptiveSeqlenFusePass::ApplyAdaptiveSeqlenPassV3(
  ...
  auto handler = [&](const GraphPatternDetector::subgraph_t& subgraph,
    ...
    op_desc.SetOutput("SeqLod", {seq_lod_name});
    op_desc.SetOutput("PadSeqLen", {max_seq_len_name});
    ...
    auto* sequence_unpad_xpu = graph->CreateOpNode(&new_op_desc);
    ...
    IR_NODE_LINK_TO(x, sequence_unpad_xpu);
    IR_NODE_LINK_TO(seq_len, sequence_unpad_xpu);
    IR_NODE_LINK_TO(sequence_unpad_xpu, x_vsl);
